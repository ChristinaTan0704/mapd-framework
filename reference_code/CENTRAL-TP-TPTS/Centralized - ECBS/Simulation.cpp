...
void Simulation::AssignTasks(vector<Agent*> &agents, const vector<vector<int> > &cons_paths)
{
	...
	for (list<Task*>::iterator it = tasks_assign.begin(); it != tasks_assign.end(); it++)
	{
		...
		if (hold[(*it)->start->loc] == false /*&& hold[(*it)->goal->loc] == false*/)
		{
			tasks.push_back(*it);
...
void Simulation::run(double focal_w)
{
	...
	for (timestep = 0; timestep <= t_task || !tasks_assign.empty() || !tasks_deliver.empty() ; timestep++)
	{
		// clock_t task_planning_time = clock();
		// clock_t path_planning_time_1 = clock();
		// clock_t path_planning_time_2 = clock();
		...
		for (list<Task*>::iterator it = tasks_deliver.begin(); it != tasks_deliver.end();)
		{
			if (true == (*it)->delivering && timestep == (*it)->ag_arrive_goal)
			{
				...
				nonidlecost += (*it)->ag_arrive_goal - (*it)->ag_arrive_start;
		...
		bool has_new_agent = false;
		for (list<Task*>::iterator it = tasks_assign.begin(); it != tasks_assign.end();)
		{
			if (ag_loc[(*it)->start->loc] >= 0) {
				//int id = ag_loc[(*it)->start->loc];
				//printf("%d %d %d %d\n", id, (*it)->start->loc, (*it)->goal->loc, hold[(*it)->goal->loc]);
				has_new_agent = true;
			}
			if (ag_loc[(*it)->start->loc] >= 0 && hold[(*it)->goal->loc] == 0 && DeliverGoal[(*it)->goal->loc] == false)// assign agent to  deliver package
			{
				int id = ag_loc[(*it)->start->loc];
		...
		if (!ag_pathfinding.empty()) //path finding
		{
			...
			//	printf("[%d %d %d]\n", ag_pathfinding[i]->id, ag_pathfinding[i]->loc, ag_pathfinding[i]->next_ep->loc);
			//}
			...
			path_planning_runtime += (double)(std::clock() - path_planning_time_1) * 1.0/ (CLOCKS_PER_SEC/1000);
		}
		...
		*/
		clock_t task_planning_time = clock();
		if (!ag_assign.empty() && has_new_agent) //assign tasks
		{
			cons_paths.clear();
			for (int i = 0; i < agents.size(); i++) {
				bool flag = true;
		...
		if (!ag_pathfinding.empty()) //path finding
		{
			...
			//printf("%d\n", ag_pathfinding.size());
			PathFinding(ag_pathfinding, cons_paths);
			path_planning_runtime += (double)(std::clock() - (double)path_planning_time_2) * 1.0/ (CLOCKS_PER_SEC/1000);
		
		}

		computation_time += (std::clock() - start);
		// cout << path_planning_runtime<< endl;
		...
		for (int i = 0; i < agents.size(); i++)
		sumofcost += agents[i].finishtime;
	//printf("%d %d\n", sumofcost, sumofcost - nonidlecost);
}

void Simulation::ShowTask(double t)
{
	WaitingTime = 0;
	LastFinish = 0;
	int flowtime = 0;
	...
	cout << "Sum of Task Waiting Time:	" << (double)WaitingTime/500 << endl;
	cout << "runtime per timestep:	" << (double)(std::clock() - t) * 1.0/ (CLOCKS_PER_SEC/1000)/LastFinish << endl;
	// printf("%d %d %d ", LastFinish, sumofcost, sumofcost - nonidlecost);
}
...
{
	// write output file
	std::ofstream fout(fname + ".txt");
	if (!fout) return;
	//fout << mPanel->agents.size() << std::endl;
	// vector<int> thpts(2500, 0);
	// vector<int> inpts(2500, 0);
	vector<int> thpts(3000, 0);
	vector<int> inpts(3000, 0);
...
void Simulation::SaveTime(const string &fname)
{
	// write output file
	std::ofstream fout(fname + ".txt");
	if (!fout) return;
    ...
    {
        fout << k << " ";
        fout << task_planning_time_list[k] << " ";
        fout << path_planning_time_list[k] << " ";
        fout << std::endl;
    }
    fout.close();
}
...
void Simulation::SavePath(string fname)
{
	...
	for (unsigned int i = 0; i < this->agents.size(); i++)
	{
		...
		for (unsigned int j = 0; j < maxtime; j++)
		{
			...
			fout << x << "	" << y << endl;
			// fout << this->agents[i].path[j] << endl;
		}
	}
	fout.close();
}
...
bool Simulation::FullCollisionCheck(const string &alg_name)
{
	bool ok = true;
	...
	for (int a1 = 0; a1 < num_agents; a1++) {
		for (int a2 = a1 + 1; a2 < num_agents; a2++) {
			for (int t = 0; t < T; t++) {
				if (agents[a1].path[t] == agents[a2].path[t]) {
					if (collision_count < 10)
						cout << "[" << alg_name << "] VERTEX COLLISION: agents "
							 << a1 << " and " << a2
							 << " at loc " << agents[a1].path[t]
							 << " time " << t << endl;
					ok = false;
					collision_count++;
				}
				if (t > 0 &&
					agents[a1].path[t] == agents[a2].path[t-1] &&
					agents[a1].path[t-1] == agents[a2].path[t]) {
					if (collision_count < 10)
						cout << "[" << alg_name << "] EDGE COLLISION: agents "
							 << a1 << " and " << a2
							 << " on edge " << agents[a1].path[t-1]
							 << "-" << agents[a1].path[t]
							 << " time " << t << endl;
					ok = false;
					collision_count++;
	...
	if (ok)
		cout << "[" << alg_name << "] COLLISION CHECK PASSED - all paths are collision-free" << endl;
	else
		cout << "[" << alg_name << "] COLLISION CHECK FAILED - " << collision_count << " collisions found" << endl;
	return ok;
}
