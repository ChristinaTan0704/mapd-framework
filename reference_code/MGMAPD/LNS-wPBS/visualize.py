...
class Animation:
    def __init__(self, my_map, starts, goals, paths, throughput):
        ...
        for i in range(len(self.paths)):
            name = str(i)
            self.agents[i] = Circle((starts[i][0], starts[i][1]), 0.3, facecolor=Colors[4],
                                    edgecolor='black')
            # self.agents[i].original_face_color = Colors[4]
        ...
        ax = plt.gca()
        plt.title('Assign task sequence', fontdict = {'fontsize' : 22}, x=0.5, y=1.1)
    ...
    def save(self, file_name, speed):
        ...
        self.animation.save(
            ...
            fps=10 * speed,
            dpi=200,
            savefig_kwargs={"pad_inches": 10, "bbox_inches": "tight"})
    ...
    def animate_func(self, t):
        ...
        if t/10 >= len(self.throughput):
            self.text.set_text("Timestep {}: {} tasks finished.".format(t/10, self.throughput[len(self.throughput)-1]))
        else:
