...
Path StateTimeAStar::updatePath(const StateTimeAStarNode* goal, const State& start)
{
...
Path StateTimeAStar::updatePathAndDummyPath(const StateTimeAStarNode* goal, const State& start, const Path path)
{
	...
	const vector<pair<int, int> >& goal_location, ReservationTable& rt)
{
    ...
    while (!focal_list.empty())
    {
		...
		if (curr->goal_id == (int)goal_location.size() || curr->state.timestep >= start.timestep + 10)
		{
