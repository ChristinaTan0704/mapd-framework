			...
			if (G.types[curr.location] != "Magic")
			{
				for (int j = k + 1; j < num_of_drives; j++)
				{
					for (int i = max(0, t - k_robust); i <= min(t + k_robust, end_timestep); i++)
					{
						...
						{
							// cout << "Drive " << remained_agents[k] << " at " << curr << " has a conflict with drive " << remained_agents[j]
							// 	<< " at " << paths[j][i] << endl;
                            ...
                            //     exit(-1);
                            // }
                            // cout << "Drive " << remained_agents[k] << " has " << goal_locations[k].size() << " remaining tasks " << remained_agents[j]
							// 	<< " has " << goal_locations[j].size() << " remaining tasks" << endl;
                            // if (goal_locations[k].empty()) {
                            //     cout << "Drive " << remained_agents[k] << " is waiting at " << paths[k][i-1] << endl;
                            // }
                            // if (goal_locations[j].empty()) {
                            //     cout << "Drive " << remained_agents[j] << " is waiting at " << paths[j][i-1] << endl;
                            // }
                            // if (!goal_locations[k].empty() && !goal_locations[j].empty()) {
                            //     cout << "Drive " << remained_agents[k] << " at " << curr << " has a conflict with drive " << remained_agents[j]
							// 	    << " at " << paths[j][i] << endl;
                            //     exit(-1);
                            // }
