...
bool PBS::find_path(PBSNode* node, int agent)
{
    ...
    clock_t t = std::clock();
    initial_rt.clear();
    boost::unordered::unordered_set<int> high_priority_agents = node->priorities.get_reachable_nodes(agent);
    
    for (int j = 0; j < starts.size(); j++)
	{
        if (agent == j || high_priority_agents.find(agent) != high_priority_agents.end())
            continue;
    ...
    // cout << node->g_val << " "<< old_cost << " "<< path.size() << endl;
...
bool PBS::generate_root_node()
{
    ...
    for (int i = 0; i < num_of_agents; i++) 
	{
        ...
        // cout << starts[i] << " " << goal_locations[i].size()<< endl;
        ...
        paths[i] = &dummy_start->paths.back().second;
        dummy_start->makespan = std::max(dummy_start->makespan, paths[i]->size() - 1);
        // cout << "dummy_start->h_val: " << path_planner.temp_h_val << endl;
                    ...
                    int time_limit)
{
    ...
    {// found a solution (and finish the while look)
        ...
        // print_paths();
        solution_cost = dummy_start->f_val;
    }
	...
	while (!dfs.empty() && !solution_found)
	{
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            if (sol)
            {
                if (screen == 2)
                {
                    // cout << n[i]->g_val << " and " << curr->g_val << endl;
