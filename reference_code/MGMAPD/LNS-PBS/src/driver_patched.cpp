...
void set_parameters(BasicSystem& system, const boost::program_options::variables_map& vm)
{
	...
	system.lns_time = vm["lns_time"].as<int>();
	if (vm.count("seed"))
...
int main(int argc, char** argv) 
{
	...
	desc.add_options()
		...
		("solver", po::value<string>()->default_value("ECBS"), "solver (LRA, PBS, WHCA, ECBS)")
		...
		("lns_time", po::value<int>()->default_value(1), "LNS optimization time in seconds (0 = Hungarian only)")
		;
	...
	if (vm["log"].as<bool>())
	{
		mkdir((vm["output"].as<std::string>() + "/goal_nodes/").c_str(), 0755);
		
		
		
	}
	if (vm["scenario"].as<string>() == "KIVAONLINE")
	{
		KivaGrid G;
		if (!G.load_Minghua_map(vm["map"].as<std::string>()))
			return -1;
		MAPFSolver* solver = set_solver(G, vm);
		KivaSystemOnline system(G, *solver);
		set_parameters(system, vm);
		G.preprocessing(system.consider_rotation);
		system.load_tasks(vm["task"].as<std::string>());
