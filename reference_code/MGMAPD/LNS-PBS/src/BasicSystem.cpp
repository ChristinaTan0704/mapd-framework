...
void BasicSystem::update_start_locations()
{
    for (int k = 0; k < num_of_drives; k++)
    {
        if (timestep < (int)paths[k].size()) {
            starts[k] = State(paths[k][timestep].location, timestep, paths[k][timestep].orientation);
        } else if (!paths[k].empty()) {
            auto last = paths[k].back();
            paths[k].resize(timestep + 1, last);
            paths[k][timestep].timestep = timestep;
            starts[k] = State(paths[k][timestep].location, timestep, paths[k][timestep].orientation);
        }
    }
}
...
void BasicSystem::save_results()
{
    ...
    for (int i : throughput_accumulate)
        output << i << "," ;
    output << std::endl;
    output.close();
    ...
    // goals
    // cout << "test" << endl;
    output.open(outfile + "\\goals.txt", std::ios::out);
    // cout << "test1" << endl;
    output << G.rows<< " "<< G.cols << " " << num_of_drives <<  std::endl;
    for (int k = 0; k < num_of_drives; k++)
    {
        // cout << "test2" << endl;
        // output << k << std::endl;
        vector<vector<int>> prev = agents_finish_task_goal_arr[k];
        for (int j = 0; j < prev.size(); j++)
        {
            for (int i = 0; i < prev[j].size(); i++)
            {
                if (i == prev[j].size()-1)
                    output << prev[j][i] << ";";
                else
    ...
    output.open(outfile + "\\plan_timestep.txt", std::ios::out);
    // cout << "test1" << endl;
    output << G.rows<< " "<< G.cols << " " << num_of_drives <<  std::endl;
    for (int k = 0; k < path_planning_timestep.size(); k++)
    {
        // cout << "test2" << endl;
        if (k != path_planning_timestep.size()-1)
            output << path_planning_timestep[k] << ",";
        else
            output << path_planning_timestep[k] << std::endl;
    }
    // output << std::endl;
    output.close();
    ...
    // time and throughput
    string task_frequency_str = std::to_string(task_frequency);
    string task_release_period_str = std::to_string(task_release_period);
    string num_of_drives_str = std::to_string(num_of_drives);
    string total_num_of_tasks_str = std::to_string(total_num_of_tasks);
    string timefile = "\\time-" + num_of_drives_str + "-" + task_frequency_str + "-" + task_release_period_str + "-" + total_num_of_tasks_str + ".txt";
    output.open(outfile + timefile, std::ios::out);
    for (int k = 0; k < task_planning_time_list.size(); k++)
    {
        output << k << " ";
        output << task_planning_time_list[k] << " ";
        output << path_planning_time_list[k] << " ";
        output << std::endl;
    }
    output.close();
    
    // save throughput
    string throughputfile = "\\throughput-" + num_of_drives_str + "-" + task_frequency_str + "-" + task_release_period_str + "-" + total_num_of_tasks_str + ".txt";
    output.open(outfile + throughputfile, std::ios::out);
    for (int k = 0; k < 3000; k++)
    {
        output << k << " ";
        int cnt = 0;
        for (int j = k; j >= max(k-99,0); j--)
        {
            if (j >= throughput_per_timestep.size())
                continue;
            else
                cnt += throughput_per_timestep[j];
        }
        output << cnt;
        output << std::endl;
    }
...
void BasicSystem::solve()
{
	...
	else // PBS or ECBS
	{
        ...
        if (REPLAN)
        {
            ...
            solver.old_paths.clear();
            solver.old_paths.resize(num_of_drives);
            ...
            for (int i = 0; i < starts.size(); i++)
			{
                ...
                solver.old_paths[i].resize(paths[i].size() - timestep);
                for (int t = 0; t < solver.old_paths[i].size(); t++)
				{
					solver.old_paths[i][t] = paths[i][timestep + t];
					solver.old_paths[i][t].timestep = t;
				}
            ...
            if (sol)
			{
                ...
                for (int i : assigned_agents)
				{
                    planned_paths[i].clear();
					planned_paths[i] = *pt;
					++pt;
				}
        ...
        //     {
        //         new_starts.clear();
        //         new_goal_locations.clear();
        //         solver.initial_rt.clear();
        //         for (int i = 0; i < starts.size(); i++)
		// 	    {   
        //             if (find(free_agents.begin(), free_agents.end(), i) != free_agents.end())
        //             {
		// 		        new_starts.emplace_back(starts[i]);
		// 		        new_goal_locations.emplace_back(goal_locations[i]);
        //             }
        ...
        //             }
		// 	    }
        //         bool sol_free = solver.run(new_starts, new_goal_locations, time_limit);
        //         if (sol_free)
		// 	    {
		// 		    auto pt = solver.solution.begin();
		...
		// 		    if (check_collisions(planned_paths))
		// 		    {
		// 		    	cout << "PLAN FOR FREE AGENTS COLLISIONS!" << endl;
		// 		    	exit(-1);
		// 		    }
		// 	    }
        //         else 
        //         {
        //             cout << "PLAN FOR FREE AGENTS FAILED!" << endl;
		// 		    	exit(-1);
        //         }
        //         update_paths(planned_paths);
        //     }
         }
		 else if (hold_endpoints || useDummyPaths)
		 {
			 vector<State> new_starts;
			 vector< vector<pair<int, int> > > new_goal_locations;
