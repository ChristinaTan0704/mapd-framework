...
bool SingleAgentICBS::findPath(bool goal_optimal, vector<PathEntry> &path, int &goal_length, double f_weight, const vector < list< tuple<int, int, bool> > >* constraints, const bool* res_table, size_t max_plan_len, double lowerbound) {
	...
	while (!focal_list.empty()) {
		...
		if (goal_optimal && vis_goal && !curr->vis_goal && goal_location != park_location)
			expand = false;;
		if (curr->vis_goal == false && curr->loc == goal_location) {
	    	curr->vis_goal = true;
	    	/*int tmax = 0;
	    	for (unsigned int ag = 0; ag < cons_paths.size(); ag++)
			{
				for (unsigned int t = curr->timestep + curr_time + 1; t < cons_paths[ag].size(); t++)
				{
