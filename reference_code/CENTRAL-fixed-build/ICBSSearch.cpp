...
bool ICBSSearch::findPathForSingleAgent(ICBSNode*  node, int ag, double lowerbound)
{
	...
	if (foundSol)
	{
		...
		for (int i = 0; i < num_of_agents; i++)
			if (!only_dummy[i])
			node->g_val += node->goal_length[i]; //max(node->g_val, costs[i][node->goal_length[i]]);
...
bool ICBSSearch::runICBSSearch() 
{
	...
	while (!focal_list.empty() && !solution_found) 
	{
		...
		if (runtime > /*TIME_LIMIT*/ 3000 || HL_num_expanded > 10000000)
		...
		if (curr->parent != NULL) {
			...
			while (u != NULL) {
				...
				if (((get<0>(*u->conflict) == get<0>(*curr->conflict) && get<1>(*u->conflict) == get<1>(*curr->conflict)) ||
					(get<0>(*u->conflict) == get<1>(*curr->conflict) && get<1>(*u->conflict) == get<0>(*curr->conflict))) &&
					get<2>(*u->conflict) == get<2>(*curr->conflict) && get<3>(*u->conflict) == get<3>(*curr->conflict) && get<4>(*u->conflict) == get<4>(*curr->conflict)) {
				for (int i = 0; i < paths[curr->agent_id]->size(); i++)
					printf("%d:%d ", i, paths[curr->agent_id]->at(i).location);
				printf("\n");
				printf("THIS ERROR\n");
				printf("%d %d %d %d\n", get<0>(u->constraint), get<1>(u->constraint), get<2>(u->constraint), u->agent_id);
				while (1);
				}
				u = u->parent;
			}
...
ICBSSearch::ICBSSearch(const vector<bool> &my_map, vector<Agent*> &agents, double f_w, const EgraphReader& egr, constraint_strategy c, 
	int cols, const vector<vector<int> > &cons_paths, int curr_time): focal_w(f_w)
{
	...
	for (int i = 0; i < num_of_agents; i++) {
		...
		search_engines[i] = new SingleAgentICBS(init_loc, goal_loc, park_loc, mymap, map_size,
			moves_offset, cols, cons_paths, curr_time, agents[0]->path.size(), agents[i]->only_dummy, agents[i]->non_dummy_path, agents[i]->start_time);
	...
	for (int i = 0; i < num_of_agents; i++) {
		...
		if (!only_dummy[i])
			dummy_start->g_val += goal_length_initially[i];//max(dummy_start->g_val, costs[i][goal_length_initially[i]]);
