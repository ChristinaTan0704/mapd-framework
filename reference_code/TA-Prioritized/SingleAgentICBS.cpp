...
bool SingleAgentICBS::findPath(bool goal_optimal, vector<PathEntry> &path, int &goal_length, double f_weight, const vector < list< tuple<int, int, bool> > >* constraints, const bool* res_table, size_t max_plan_len, double lowerbound) {
	// clear data structures if they had been used before
	// (note -- nodes are deleted before findPath returns)
	...
	while (!focal_list.empty()) {
		...
		num_expanded++;
		//if (curr->loc == park_location)
		//	printf("!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
		bool expand = true;
		if (goal_optimal && vis_goal && !curr->vis_goal && goal_location != park_location)
			expand = false;
		if (curr->vis_goal == false && curr->loc == goal_location && curr->g_val >= release_time) {
	    	curr->vis_goal = true;
	    	curr->goal_length = curr->g_val + 1;
	    	vis_goal = true;
	    }
	    //printf("%d %d %d %d\n", goal_location, park_location, curr->loc, curr->g_val + curr->h_val);
	    // check if the popped node is a park
	    if (curr->vis_goal && curr->loc == park_location && curr->timestep > lastParkConsTime) 
		{
		...
		for (int i = 0; i < 5; i++)
		{
			...
			if (0 <= next_id && next_id < map_size && abs(next_id % moves_offset[MapLoader::valid_moves_t::SOUTH] - curr->loc % moves_offset[MapLoader::valid_moves_t::SOUTH]) < 2)
			{
				...
				if (!my_map[next_id] && !isConstrained(curr->loc, next_id, next_timestep, constraints)) {
					...
					if (curr->vis_goal && goal_location != park_location) {
						next_g_val = curr->g_val + 1;
						next_h_val = my_heuristic1[next_id];
					}
...
SingleAgentICBS::SingleAgentICBS(int start_location, int goal_location, int park_location, const bool* my_map, int map_size, 
	const int* moves_offset, int num_col, const vector<vector<int> > &cons_paths, int curr_time, int max_time, int release_time):
	cons_paths(cons_paths), curr_time(curr_time), max_time(max_time), release_time(release_time)
{
	this->my_map = my_map;
