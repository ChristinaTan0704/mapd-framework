...
public:
	...
	*/
	...
	bool findPath(bool goal_optimal, vector<PathEntry> &path, int &goal_l, double f_weight, const vector < list< tuple<int, int, bool> > >* constraints, const bool* res_table, size_t max_plan_len, double lowerbound);
	inline void releaseClosedListNodes(hashtable_t* allNodes_table);
	SingleAgentICBS(int start_location, int goal_location, int park_location, const bool* my_map, int map_size, const int* moves_offset, int num_col, const vector<vector<int> > &cons_paths, int curr_time, int max_time, int release_time);
	~SingleAgentICBS();
