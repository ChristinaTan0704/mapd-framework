...
bool ICBSSearch::findPathForSingleAgent(ICBSNode*  node, int ag, double lowerbound)
{
	...
	newPath.first = ag;
	int goal_l;
	bool foundSol = search_engines[ag]->findPath(goal_optimal[ag], newPath.second, goal_l, focal_w, cons_vec, res_table, max_plan_len, 0);
	if (!foundSol) {
		goal_optimal[ag] = false;
		foundSol = search_engines[ag]->findPath(goal_optimal[ag], newPath.second, goal_l, focal_w, cons_vec, res_table, max_plan_len, 0);
	}
	...
	if (foundSol)
	{
		...
		node->g_val = node->g_val + - goal_length[ag] + goal_l; // + newPath.second.size();
		paths[ag] = &node->new_paths.back().second;
		goal_length[ag] = goal_l;
...
bool ICBSSearch::runICBSSearch() 
{
	...
	while (!focal_list.empty() && !solution_found) 
	{
		...
		if (runtime > /*TIME_LIMIT*/ 300 || HL_num_expanded > 10000000)
		...
		else
		{
			n1->agent_id = get<0>(*curr->conflict);
			n2->agent_id = get<1>(*curr->conflict);
			//printf("%d %d %d %d\n", n1->agent_id, goal_length[n1->agent_id], n2->agent_id, goal_length[n2->agent_id]);
	...
	int cols, const vector<vector<int> > &cons_paths, int curr_time): focal_w(f_w)
{
	...
	for (int i = 0; i < num_of_agents; i++) {
		...
		int goal_loc = agents[i]->next_ep->loc; 
		...
		search_engines[i] = new SingleAgentICBS(init_loc, goal_loc, park_loc, mymap, map_size,
			moves_offset, cols, cons_paths, curr_time, agents[0]->path.size(), 0);
	...
	for (int i = 0; i < num_of_agents; i++) {
		...
		goal_optimal[i] = true;
		if (search_engines[i]->findPath(goal_optimal[i], paths_found_initially[i], goal_length_initially[i], f_w, NULL, res_table, dummy_start->makespan + 1, 0) == false) {
			goal_optimal[i] = false;
			if (search_engines[i]->findPath(goal_optimal[i], paths_found_initially[i], goal_length_initially[i], f_w, NULL, res_table, dummy_start->makespan + 1, 0) == false)
