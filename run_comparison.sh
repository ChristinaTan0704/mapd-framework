...
for AG in 10 50; do
    ...
    # REF: TA-Prioritized (use driver_scale for all agent counts)
    TA_PRIO_TOUR="$TA_PRIO/tour/${AG}-500.tour"
    if [ -f "$TA_PRIO_TOUR" ] && [ -f "$TA_PRIO/driver_scale" ]; then
        echo -n "  TA-Prioritized REF..." >&2
        TA_PRIO_OUT="/tmp/ta_prio_ref_${AG}.out"
        cd "$TA_PRIO"
        gtimeout $TIMEOUT ./driver_scale "$MAP" "$TASK" "$TA_PRIO_TOUR" "$TA_PRIO_OUT" > /dev/null 2>&1
        if [ -f "$TA_PRIO_OUT" ]; then
            python3 -c "
lines=open('$TA_PRIO_OUT').readlines()
n=int(lines[0])
