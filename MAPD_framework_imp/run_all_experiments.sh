...
for NAGS in "${MAPS[@]}"; do
    for FREQ in "${FREQS[@]}"; do
        ...
        add_job "CENTRAL-CBS $NAGS $FREQ -a CENTRAL"
        add_job "CENTRAL-PBS $NAGS $FREQ -a CENTRAL --mapf PBS"
        add_job "CENTRAL-PBS-SeqMLA $NAGS $FREQ -a CENTRAL --mapf PBS --mla_mode seq"
        add_job "CENTRAL_FIXED-CBS $NAGS $FREQ -a CENTRAL_FIXED"
        add_job "CENTRAL_FIXED-PBS $NAGS $FREQ -a CENTRAL_FIXED --mapf PBS"
        add_job "CENTRAL_FIXED-PBS-SeqMLA $NAGS $FREQ -a CENTRAL_FIXED --mapf PBS --mla_mode seq"
        add_job "HBH-MLA $NAGS $FREQ -a HBH_MLA"
        add_job "HUNGARIAN-PBS $NAGS $FREQ -a HUNGARIAN_PBS"
        add_job "HUNGARIAN-PBS-SeqMLA $NAGS $FREQ -a HUNGARIAN_PBS --mla_mode seq"
        add_job "HUNGARIAN-wPBS $NAGS $FREQ -a HUNGARIAN_wPBS"
        add_job "HUNGARIAN-wPBS-SeqMLA $NAGS $FREQ -a HUNGARIAN_wPBS --mla_mode seq"
        add_job "HUNGARIAN-PP $NAGS $FREQ -a HUNGARIAN_PBS --mapf PP"
        add_job "HUNGARIAN-PP-SeqMLA $NAGS $FREQ -a HUNGARIAN_PBS --mapf PP --mla_mode seq"
        add_job "LNS1s-PBS $NAGS $FREQ -a LNS_PBS --lns_time 1"
        add_job "LNS1s-PBS-SeqMLA $NAGS $FREQ -a LNS_PBS --lns_time 1 --mla_mode seq"
        add_job "LNS1s-wPBS $NAGS $FREQ -a LNS_wPBS --lns_time 1"
        add_job "LNS1s-wPBS-SeqMLA $NAGS $FREQ -a LNS_wPBS --lns_time 1 --mla_mode seq"
        add_job "LNS1s-PP $NAGS $FREQ -a LNS_PBS --mapf PP --lns_time 1"
        add_job "LNS1s-PP-SeqMLA $NAGS $FREQ -a LNS_PBS --mapf PP --lns_time 1 --mla_mode seq"
    done
