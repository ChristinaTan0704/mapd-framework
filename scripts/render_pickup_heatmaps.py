#!/usr/bin/env python3
"""Render task-pickup density overlays for all packaged benchmark maps."""
...
OUTPUT_DIR = PACKAGE / "visualizations/pickup_heatmaps"
...
def read_pickups(path: Path, endpoint_count: int) -> tuple[Counter[int], int]:
    lines = path.read_text().splitlines()
    ...
    for task_id, line in enumerate(lines[1:]):
        ...
        if not 0 <= pickup < endpoint_count:
            raise ValueError(f"{path}: invalid pickup endpoint {pickup}")
        pickups[pickup] += 1
    return pickups, task_count
...
def raster(grid: list[str], endpoints: list[tuple[int, int]],
           pickups: Counter[int]) -> tuple[Image.Image, int]:
    ...
    maximum = max(pickups.values(), default=0)
    if maximum:
        for endpoint_id, count in pickups.items():
...
def render_panel(title: str, map_path: Path, task_path: Path,
    ...
    pickups, task_count = read_pickups(task_path, len(endpoints))
    source, maximum = raster(grid, endpoints, pickups)
    ...
    detail = (
        ...
        f"{task_count:,} tasks  •  max {maximum} pickups/cell")
    ...
    legend = [
        ...
        (BASE_COLORS["e"], "0 pickups"),
        (heat_color(1, max(1, maximum)), "Low"),
        (heat_color(maximum, max(1, maximum)), "Most pickups"),
    ]
    item_width = 150
...
def main() -> None:
    ...
    for benchmark in BENCHMARKS:
        for agents in benchmark.agent_counts:
            ...
            output = OUTPUT_DIR / (
                f"benchmark_{benchmark.name}_a{agents}_pickup_heatmap.png")
    ...
    draw.text((comparison_width / 2, 32), "Task pickup-location density",
              font=TITLE_FONT, fill="#17191c", anchor="mm")
    draw.text((comparison_width / 2, 65),
              "Darker endpoint cells received more pickups",
    ...
    comparison_path = (
        PACKAGE / "visualizations/benchmark_pickup_heatmaps_comparison.png")
