...
void Simulation::init(const string& map_file, const string& task_file, const MAPDConfig& cfg,
    ...
    // Reproduce the reference MGMAPD agent-home shuffle for ALL methods.
    ...
    // agent k started at a different cell -> different priority/planning order.
    // Apply the identical shuffle here unconditionally (per user request:
    // shuffle for every algorithm, not just the MGMAPD Hungarian/LNS methods).
